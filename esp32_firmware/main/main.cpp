...
// Ensure CONFIG macros are defined, or provide defaults to avoid compilation failure
...
static esp_err_t init_wifi() {
    ...
    ESP_RETURN_ON_FALSE(wifi_event_group, ESP_ERR_NO_MEM, TAG, "Failed to create event group");
...
static void acquisition_task(void *pvParameters) {
    ...
    while (true) {
        ...
        bool lo_neg = false;
#ifdef CONFIG_ECG_LO_PLUS_GPIO
        lo_pos = (CONFIG_ECG_LO_PLUS_GPIO != -1) ? gpio_get_level((gpio_num_t)CONFIG_ECG_LO_PLUS_GPIO) : false;
#endif
#ifdef CONFIG_ECG_LO_MINUS_GPIO
        lo_neg = (CONFIG_ECG_LO_MINUS_GPIO != -1) ? gpio_get_level((gpio_num_t)CONFIG_ECG_LO_MINUS_GPIO) : false;
#endif
...
/**
 * @brief Publishing Task: Batches samples and publishes via binary MQTT protocol
 */
static void publishing_task(void *pvParameters) {
    uint32_t sequence = 0;
    float batch[BATCH_SIZE];
    uint8_t packet_buf[sizeof(ecg::EcgPacketHeader) + (CONFIG_ECG_MQTT_BATCH_SIZE * 4) + 1];
    ...
    while (true) {
        ...
        ecg::EcgPacketHeader *hdr = reinterpret_cast<ecg::EcgPacketHeader *>(packet_buf);
        hdr->magic = 0xA5;
        ...
        esp_mqtt_client_publish(mqtt_client, CONFIG_ECG_MQTT_TOPIC,
                                reinterpret_cast<const char *>(packet_buf), data_len + 1,
                                qos, 0);
#endif
        ...
        if (CONFIG_ECG_LOG_EVERY_N_SAMPLES > 0 &&
            ...
            ESP_LOGI(TAG, "Stream active: Sent batch %lu (Total: %lu samples)", (unsigned long)sequence, (unsigned long)(sequence * BATCH_SIZE));
        }
#endif
    }
}
